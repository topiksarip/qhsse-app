    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200">{editing ? 'Edit Dokumen' : 'Buat Dokumen'}</h2>}>
            ...
            <div className="py-10"><form className="mx-auto max-w-4xl space-y-6 px-4 sm:px-6 lg:px-8">
                ...
                <section className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800"><h3 className="text-lg font-semibold text-slate-900 dark:text-white">File Terkontrol</h3><p className="mt-1 text-sm text-slate-500">PDF, DOC/DOCX, XLS/XLSX, atau PPT/PPTX. Maksimum 50 MB. Disimpan pada private storage.</p><input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" onChange={(e) => setData('file', e.target.files?.[0] ?? null)} className="mt-4 block w-full rounded-lg border border-dashed border-slate-300 p-4 text-sm dark:border-gray-600 dark:text-slate-300" /><Error name="file" /></section>
