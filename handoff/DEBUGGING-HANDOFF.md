...
## 4b. D6 Closed — Real `GenerateReportJob` Bug Found & Fixed

During the D6 end-to-end proof (seed `ReportTemplate` + `SavedReport`, dispatch
`GenerateReportJob` through the **real queue worker**), the job failed with:

```
TypeError: GenerateReportJob::generateSafetyMetricsCsv():
  Argument #1 ($params) must be of type array, string given
```

**Root cause:** `SavedReport::$parameters` is cast to `array`, but after the
job is rehydrated from the queue (`SerializesModels`), `$this->report->parameters`
arrives as a **JSON string**, not an array. Every type-specific generator
(`generateIncidentSummaryCsv`, `generateSafetyMetricsCsv`, etc.) has an
`array $params` signature, so the call type-errors and the job is marked
`failed`. This is a **latent production bug** (the UI sync path happened to
work because the in-request model kept the cast, but the async queue path did
not).

**Fix** (`app/Jobs/Modules/Reporting/GenerateReportJob.php`, `generateCsvContent()`):
normalize `$params` defensively before dispatch to the generators:
```php
...
**Proof (queue path, not sync):** after redeploying the fix and restarting the
worker, dispatched a fresh `SavedReport` → worker picked it up (`jobs: 1→0`),
status flipped `pending → completed`, `file_path` + `file_size=195` set, and the
CSV file existed on disk (`reports/6/D6_Queue_Proof_....csv`). Synchronous
execution (`$job->handle()`) also yields `completed`. Proof rows were deleted
afterwards; no leftover test data.

**Impact:** Saved Reports generated asynchronously (the intended production
flow) now complete instead of failing. This was the real D6 defect — not the
queue infrastructure, which was already verified working.
